...
app.get("/GetWatchlist/:userId", async (q, s) => {
    ...
    let watchlist: GetWatchlistResponse = { entries: [] };
...
app.get(
  "/GetPriceHistory/:currencyCode/:startDate/:endDate",
  async (request, response) => {
    ...
    const data: any = await (await fetch(GetPriceHistoryAPIURL)).json();
...
type HistoricalData = CandlePrice[];


type AlphaVantageExchangeRateResponse = {
...
export async function alphaVantage(fromCurrencyCode:string) {
    ...
    const originalResponse: AlphaVantageExchangeRateResponse  = await response.json() as any;
